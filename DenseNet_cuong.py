...
class DenseNet:
    ...
    # marginal loss
    def marginal_loss(self,m, thre = 1.2, slack = 0.3):
        # m = self.centers_batch
        # distance
        A = self.score #feature

        A_norm = tf.nn.l2_loss(A)
        A = tf.div(A, A_norm)
        ...
        matrix_sub = (slack - tf.matmul(C, thre - D))
        return tf.reduce_sum(tf.where(tf.less(matrix_sub,0.0), 0))
    ...
    def losses(self):
        ...
        self.marginal_loss(m=self.margins_batch )
